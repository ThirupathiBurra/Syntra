...
export function Sidebar() {
  ...
  return (
    <aside className={`${isCollapsed ? 'w-16' : 'w-64'} border-r border-zinc-800 bg-zinc-950/50 backdrop-blur-xl hidden md:flex flex-col h-full transition-all duration-300 relative group`}>
      <button 
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -right-3 top-4 h-6 w-6 bg-zinc-900 border border-zinc-700 rounded-full flex items-center justify-center text-zinc-400 hover:text-zinc-100 opacity-0 group-hover:opacity-100 transition-opacity z-50"
      >
        {isCollapsed ? <PanelLeftOpen className="h-3 w-3" /> : <PanelLeftClose className="h-3 w-3" />}
      </button>

      <div className="flex h-14 items-center px-4 border-b border-zinc-800/50 overflow-hidden">
        <Compass className="h-6 w-6 text-indigo-500 mr-2 flex-shrink-0" />
        <span className={`font-semibold text-lg tracking-tight text-zinc-100 whitespace-nowrap transition-opacity duration-300 ${isCollapsed ? 'opacity-0 w-0 hidden' : 'opacity-100'}`}>Syntra OS</span>
      </div>
        ...
        <nav className="space-y-1 px-2">
          {navItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="flex items-center px-3 py-2 text-sm font-medium rounded-md text-zinc-400 hover:text-zinc-50 hover:bg-zinc-800/50 transition-colors"
              title={item.name}
            >
      ...
      <div className="p-4 border-t border-zinc-800/50 overflow-hidden">
        <div className="flex items-center">
          ...
          <div className={`ml-3 whitespace-nowrap transition-opacity duration-300 ${isCollapsed ? 'opacity-0 w-0 hidden' : 'opacity-100'}`}>
            <p className="text-sm font-medium text-zinc-200">System Admin</p>
